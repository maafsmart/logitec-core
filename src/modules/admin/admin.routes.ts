...
import { createPdaPairingChallenge } from "../pda/pda-auth.service.js";
import { forceTakeoverPdaSession } from "../pda/pda-run.service.js";
...
adminRouter.post(
  "/pda-test-sessions/:sessionId/pairing",
  pdaScannerLabApiGate,
  requireAuth,
  requireRole(["ADMIN"]),
  requireOperationalClient,
  async (req, res) => {
    ...
    const result = await createPdaPairingChallenge({
      clientId: req.auth!.operationalClientId!,
      userId: req.auth!.userId,
      sessionId
    });
...
adminRouter.post(
  ...
  async (req, res) => {
    ...
    void body;
    res.setHeader("Cache-Control", "no-store");
    res.json(await forceTakeoverPdaSession(req.auth!.operationalClientId!, sessionId));
  }
);
