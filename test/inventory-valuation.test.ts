...
test("valuación visible para consulta ADMIN/SUPERVISOR/OPERATOR/CLIENT; solo ADMIN edita", () => {
  ...
  assert.equal(publicValuationForRole(valuation, canExposeEconomicValuation("SUPERVISOR"))?.totalValueMxn, "3.00");
  assert.equal(publicValuationForRole(valuation, canExposeEconomicValuation("CLIENT"))?.totalValueMxn, "3.00");
});
