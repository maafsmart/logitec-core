...
var state = { role: "SUPERVISOR", operatorMode: false, provisionalCaptures: [] };
...
test("Operador CP validada desde Pendientes registra Validación de Supervisor", () => {
  const updated = apply(
    baseCapture({ executor: "Operador", executorRole: "OPERATOR" }),
    "VALIDADO · PENDIENTE DE REGISTRO"
  );
...
test("Supervisor directo validado desde Pendientes registra Autovalidación", () => {
  const updated = apply(
    baseCapture({ executor: "Supervisor", executorRole: "SUPERVISOR" }),
    "VALIDADO · PENDIENTE DE REGISTRO"
  );
