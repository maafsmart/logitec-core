...
- Al abrirse, la cámara pasa por `Preparando cámara` y luego queda en `Cámara lista para enfocar`: mantiene el preview activo, pero no ejecuta detección. El usuario encuadra y enfoca el código antes de pulsar `INICIAR LECTURA`; solo ese clic cambia a `Lectura armada`, fija el origen de tiempo y activa el detector. Tras una lectura, el detector vuelve a quedar desarmado y el preview sigue visible para preparar la siguiente.
- Mientras la lectura está armada, cada intento del detector usa un canvas congelado del preview. Solo cuando ese mismo canvas produce un código se convierte en `Blob` y se muestra junto al resultado mediante una URL local revocable; así la evidencia corresponde al frame efectivamente analizado. La evidencia vive únicamente en memoria de la pestaña: no se agrega al historial/CSV, no usa storage, no se envía a la API ni se guarda en base de datos. La siguiente detección revoca y reemplaza la imagen anterior; el usuario también puede descartarla.
