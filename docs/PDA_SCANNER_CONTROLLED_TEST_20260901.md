...
- Al detectar, el navegador congela el frame visible en un `Blob` y lo muestra junto al resultado mediante una URL local revocable. La evidencia vive únicamente en memoria de la pestaña: no se agrega al historial/CSV, no usa storage, no se envía a la API ni se guarda en base de datos. La siguiente detección revoca y reemplaza la imagen anterior; el usuario también puede descartarla.
