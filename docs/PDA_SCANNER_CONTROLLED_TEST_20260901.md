...
- El historial vive solo en memoria de la pestaña y puede copiarse o exportarse a CSV; si una lectura de cámara retuvo su frame, la exportación local incluye esas imágenes y un manifiesto en un ZIP generado en el navegador. No se crea ningún `ScanEvent` ni otro registro de base de datos.
...
- Cámara requiere HTTPS (o localhost) y permiso explícito al presionar `Abrir cámara`. Chrome Android y Samsung Internet usan `BarcodeDetector` nativo cuando está disponible. Safari iPhone, Firefox y otros navegadores sin esa API cargan bajo demanda `barcode-detector` 3.2.2 + ZXing-WASM desde el propio servidor (sin CDN; ~43 KB JS y ~1.1 MB WASM).
- Al abrirse, la cámara pasa por `Preparando cámara` y luego queda en `Cámara lista para enfocar`: mantiene el preview activo, pero no ejecuta detección. El usuario encuadra y enfoca el código antes de pulsar `INICIAR LECTURA`; solo ese clic cambia a `Lectura armada`, fija el origen de tiempo y activa el detector. Tras una lectura, el detector vuelve a quedar desarmado, se reactiva `INICIAR LECTURA` y el preview sigue visible para preparar la siguiente, sin detener la cámara.
- Mientras la lectura está armada, cada intento del detector usa un canvas congelado del preview. Solo cuando ese mismo canvas produce un código se convierte a data URL JPEG local y se muestra junto al resultado; así la evidencia corresponde al frame efectivamente analizado. `showDetectedFrame` es síncrona: no se encadena `.catch` sobre su retorno. La evidencia vive únicamente en memoria de la pestaña; cada entrada de sesión puede retener su frame hasta cerrar la pestaña. No usa storage persistente, no se envía a la API ni se guarda en base de datos. La evidencia visible actual se reemplaza en la siguiente detección o al descartarla; el historial conserva las copias de la sesión.
- Tras detectar un código, el laboratorio intenta una vibración breve (`navigator.vibrate`) y un beep local con Web Audio. Si el navegador no lo soporta o falla, la clasificación continúa.
- El fallback manual/lector teclado siempre permanece disponible. La cámara se detiene al cambiar de modo, ocultar o cerrar la pestaña, o por acción explícita. Compatibilidad esperada no significa rendimiento idéntico: enfoque, iluminación, resolución y formato físico deben validarse con los equipos reales.
- Las métricas separan `tiempo hasta detección` (desde `INICIAR LECTURA` hasta recibir `rawValue`) de `latencia de clasificación API` (duración exclusiva del GET de clasificación). El tiempo negro/preparación y el enfoque previo quedan excluidos; las lecturas manuales no inventan tiempo de detección.
...
## Siguiente fase propuesta: Modo compañero Lap ↔ Motorola

Esta fase queda documentada, no implementada ni habilitada en esta ronda.

- La laptop continuaría operando el WMS en su propia sesión; el Motorola sería un `lector compañero` que usa su cámara y decodifica el código en el teléfono.
- No se convertiría al Motorola en webcam ni se transmitiría video al navegador de la laptop. `Cámara/lector remoto compañero` y `webcam remota` son arquitecturas distintas; solo se propone la primera.
- El emparejamiento debería ser explícito y temporal, por ejemplo mediante QR o código de un solo uso generado por la laptop, con expiración corta, una sola pareja activa y revocación al cerrar cualquiera de las sesiones.
- Un canal cifrado y autenticado de vida corta podría retransmitir únicamente el valor decodificado, un identificador de sesión no reutilizable y metadatos técnicos mínimos. El frame no debería enviarse por defecto.
- El servidor tendría que validar usuario, rol, cliente operativo y vínculo de emparejamiento en cada mensaje, además de impedir replay, cruce entre clientes y reutilización del canal.
- La laptop mostraría el código recibido para confirmación; conectarlo con campos o acciones productivas requerirá una revisión separada de permisos, auditoría, idempotencia y seguridad.
- No se propone en esta fase ninguna escritura de inventario, picking, movimientos ni automatización productiva.
